...
class DQNEnvironment:
    ...
    def update_environment(self):
        ...
        if self.memory['S2'][1] == 0 and self.memory['S1'][1] == 0:  # Gripper has remained closed
            if self.memory['action'] in [2, 3]:
                reward -= 1  # Penalty for wrong rotation with gripper closed
            if self.memory['action'] == 0:
                reward += 1 - 2 * self.memory['S2'][0]  # Reward for correct rotation with gripper closed
            if self.memory['action'] == 1 and self.memory['S2'][4]:
                reward += 0.2  # Reward for unthreading after cross-threading detection

        if self.memory['S2'][1] == 1 and self.memory['S1'][1] == 1:  # Gripper has remained open
            if self.memory['action'] in [0, 1]:
                reward -= 1  # Penalty for wrong rotation with gripper open
            if self.memory['action'] == 2 and self.memory['S1'][3] >= 1:
                reward += 0.2  # Reward for turning arm -120 when at positive limit
            if self.memory['action'] == 3 and self.memory['S1'][3] <= -1:
                reward += 0.2  # Reward for turning arm +120 when at negative limit
            
        # Goal achievement
        if self.memory['S2'][1] == 1 and (self.memory['S2'][2] % (240/600)) == 0 and self.memory['action'] == 4:
            reward += 5  # 240 degrees reached, nut released
